...
const ProductsContainer = ({ products }) => {
  ...
  const [
    isProductCustomizeMealActive,
    setIsProductCustomizeMealActive,
  ] = useState(false);
  ...
  return (
    <div>
      ...
      <ProductModalCustomizeMeal
        isActive={isProductCustomizeMealActive}
        productDetails={productDetails}
        close={() => setIsProductCustomizeMealActive(false)}
      />
      <div className="row">
        {products.map((product) => (
          <div className="col-md-4" key={product.id}>
            <ProductCard
              openMoreDetails={() => openProductSlider(product.id)}
              product={product}
            />
          </div>
        ))}
